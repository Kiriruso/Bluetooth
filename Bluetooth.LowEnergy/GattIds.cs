...
namespace Bluetooth.LowEnergy
{
    public static class GattIds
    {
        ...
        private static bool IsStandardUuid(string uuid)
        {
            return Services.ContainsKey(uuid) || Characteristics.ContainsKey(uuid);
        }

        private static string UuidToString(Guid uuid)
        {
            var uuid16 = "0x" + uuid.ToString("N").Substring(4, 4).ToUpper();
            return IsStandardUuid(uuid16) ? uuid16 : uuid.ToString();
        }

        public static (string, string, bool) GetInfo(this GattDeviceService service)
        {
            var uuid = UuidToString(service.Uuid);
            var name = IsStandardUuid(uuid) ? Services[uuid] : "Unknown";
            var canMaintain = service.Session.CanMaintainConnection;
            return (uuid, name, canMaintain);
        }

        public static (string, string, GattCharacteristicProperties, GattProtectionLevel) GetInfo(this GattCharacteristic characteristic)
        {
            var uuid = UuidToString(characteristic.Uuid);
            var name = IsStandardUuid(uuid) ? Characteristics[uuid] : "Unknown";
            var properties = characteristic.CharacteristicProperties;
            var protectionLevel = characteristic.ProtectionLevel;
            return (uuid, name, properties, protectionLevel);
        }
    }
}